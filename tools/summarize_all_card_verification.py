...
def main():
    ...
    for batch in ['basic_card', 'advanced_card', 'magic', 'items']:
        ...
        for case in effects['cases']:
            ...
            assert case['status'] == 'failed' if failed or case.get('exception') else case['status'] in ['passed', 'partial']
    ...
    text = ['# 原版卡全库首轮场景审计', '', f"范围：{summary['cardCount']} 条现有行动/法术/宝物卡资产，{summary['caseCount']} 个案例。", '',
            ...
            f"案例结果：{summary['caseResults']}。卡片结果：{summary['cardResults']}。", '', '| 卡牌 ID | 首轮结论 | 案例数 |', '|---|---|---:|']
